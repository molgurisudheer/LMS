class LeavesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @leave.save
        LeaveMailer.with(user: current_user, reporting_head: User.reporting_heads.first).new_leave_request.deliver_now
        LeaveMailer.with(user: current_user, reporting_head: User.reporting_heads.first).new_leave_request_submitted.deliver_now
  ...
  def update
    respond_to do |format|
      if @leave.update(leave_params)
        LeaveMailer.with(user: current_user).modified_leave_request.deliver_now
  ...
  def destroy
    @leave.destroy
    LeaveMailer.with(user: current_user).cancelled_leave_request.deliver_now
    respond_to do |format|
